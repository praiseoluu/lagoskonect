...
export default class ReelsPage extends WebLayout {
  ...
  async onContentReady() {
    ...
    this.delegate('.reel-card[data-reel-id]:not(.reel-ad-card)', 'click', (e, card) => {
      ...
      router.push(`/east/reels/${card.dataset.reelId}`);
    });
  }
      ...
      card.innerHTML = `
        <div class="reel-card__thumb-wrap">
          ...
          <div class="reel-card__author">
            ${avatarHtml}
            <a class="reel-card__author-name" href="/east/u/${reel.authorHandle ? this.esc(reel.authorHandle.replace('@','')) : this.esc(reel.authorName)}">${this.esc(reel.authorName)}</a>
          </div>
        </div>
